...
type EventListItemHybridBProps = {
	event: Event;
};

/**
 * Hybrid B: Mini Timeline + Dense Card
 * Smaller date badge, shorter connector, dense card with thumbnail on top-left
 */
export default React.memo(function EventListItemHybridB({ event }: EventListItemHybridBProps) {
	...
	return (
		<div className="flex gap-3 group">
			{/* Small Timeline Date Badge */}
			<div className="flex-shrink-0 pt-1">
				{dateInfo ? (
					<div className="text-right">
						<div className="text-sm font-medium text-neutral-600">{dateInfo.month}</div>
						<div className="text-2xl font-bold text-primary-600 leading-none">{dateInfo.day}</div>
					</div>
				) : (
					<Calendar className="w-8 h-8 text-neutral-400 mt-2" />
				)}
			</div>

			{/* Short Timeline Connector */}
			<div className="flex flex-col items-center pt-5 pb-3">
				<div className="w-2 h-2 rounded-full bg-primary-500 border-2 border-white shadow-sm flex-shrink-0"></div>
				<div className="w-px bg-neutral-200 flex-1"></div>
			</div>
