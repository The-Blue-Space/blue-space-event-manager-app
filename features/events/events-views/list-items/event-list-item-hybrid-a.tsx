...
type EventListItemHybridAProps = {
	event: Event;
};

/**
 * Hybrid A: Timeline + Compact Card
 * Large date badge, timeline connector, dense card layout
 */
export default React.memo(function EventListItemHybridA({ event }: EventListItemHybridAProps) {
	...
	return (
		<div className="flex gap-4 group">
			{/* Large Timeline Date Badge */}
			<div className="flex-shrink-0 w-20 pt-1">
				{dateInfo ? (
					<div className="text-center">
						<div className="text-3xl font-bold text-primary-600 leading-none">{dateInfo.day}</div>
						<div className="text-sm font-medium text-neutral-600 uppercase mt-1">
							{dateInfo.month}
						</div>
						<div className="text-xs text-neutral-500">{dateInfo.year}</div>
					</div>
				) : (
					<div className="w-12 h-12 rounded-full bg-neutral-200 flex items-center justify-center mx-auto">
			...
			{/* Timeline Connector */}
			<div className="flex-shrink-0 w-px bg-neutral-200 relative">
				<div className="absolute top-6 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-primary-500 border-2 border-white shadow-sm"></div>
			</div>

			{/* Event Card */}
			<div className="flex-1 pb-6">
				<div className="relative border border-neutral-200 rounded-lg hover:border-neutral-300 hover:shadow-md transition-all bg-white">
					{/* Action buttons */}
						...
						<div className="flex-1 min-w-0 space-y-2">
							...
							<div className="flex items-start justify-between gap-3 pr-8">
								...
								<Badge
									...
									className={
										event.published
											? "bg-green-500 hover:bg-green-600 text-white flex-shrink-0"
							...
							{/* Metrics Row */}
							<div className="flex items-center gap-1.5 flex-wrap pt-1.5 border-t border-neutral-100">
								<Badge
									variant="secondary"
