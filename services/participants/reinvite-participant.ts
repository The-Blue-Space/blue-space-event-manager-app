...
export async function production(params: Parameters): Promise<Response> {
	const response = await axios.patch(`/v1/participants/${params.participant_id}/reinvite`);
...
export async function development(params: Parameters): Promise<Response> {
	...
	console.log(`Mock: Reinvited participant ${participant_id}`);
...
export default async function reinviteParticipant(params: Parameters): Promise<Response> {
